...
# Include your other FastAPI endpoint definitions below
# Example endpoint, replace with your actual endpoint implementations:
...
async def load_data():
    global uploaded_df
    file_path = os.path.join(os.path.dirname(__file__), "D:\Election sentiment analysis (Fast api)\data\pre_election_processed_dates.xlsx")  # Set the path to your Excel file here
...
async def preprocess_data():
    ...
    try:
        ...
        # Remove duplicates based on the 'cleaned_tweet' column
        original_count = len(uploaded_df)
        cleaned_df = uploaded_df.drop_duplicates(subset=['cleaned_tweet'], keep='first')
        processed_count = cleaned_df.shape[0]

        # Prepare results for JSON response
        results = {
            "original_count": original_count,
            "processed_count": processed_count,
            "sample_processed": cleaned_df[['Tweet', 'cleaned_tweet']].head().to_dict()
        }

        # Define the file path and save the cleaned DataFrame to an Excel file
        save_path = os.path.join(SAVE_DIR, "cleaned_tweets.xlsx")
        cleaned_df.to_excel(save_path, index=False, sheet_name="Cleaned_Tweets")
...
async def label_tweets() -> JSONResponse:  # Change return type to JSONResponse
    """
    Label tweets with party affiliations using the preprocessed file.
    """
    try:
        # Load the preprocessed file from the specified directory
        preprocessed_file_path = os.path.join(SAVE_DIR, "cleaned_tweets.xlsx")
        ...
        # Define the file path and save the labeled DataFrame to an Excel file
        save_path = os.path.join(SAVE_DIR, "labeled_tweets.xlsx")
        df.to_excel(save_path, index=False, sheet_name="Labeled_Tweets")
        
        # Add the file path to results
        results["file_saved_path"] = "data/labeled_tweets.xlsx"
...
async def annotate_sentiment():
    ...
    """
    try:
        labeled_file_path = os.path.join(SAVE_DIR, "labeled_tweets.xlsx")
        ...
        output_file_path = os.path.join(SAVE_DIR, "labeled_tweets_with_sentiments.xlsx")
        with pd.ExcelWriter(output_file_path, engine='xlsxwriter') as writer:
            df.to_excel(writer, index=False, sheet_name='Vader_Sentiments')
...
async def analyze_sentiments() -> Dict[str, Any]:
    try:
        ...
        labeled_file_path = os.path.join(SAVE_DIR, "labeled_tweets_with_sentiments.xlsx")
...
async def compare_sentiment_actual():
    ...
    try:
        ...
        tweet_file_path = os.path.join(SAVE_DIR, "labeled_tweets_with_sentiments.xlsx")
        ...
        # Define the updated party mapping, including 'IND' and 'ppp'
        party_mapping = {
...
async def post_election_analysis():
    try:
        # Define file paths
        tweet_file_path = os.path.join(SAVE_DIR, "post election dataset with processed_dates.xlsx")
        ...
        # Save results
        df.to_excel('./data/post_election_sentiment_analysis.xlsx', index=False)
...
async def compare_pre_post_election():
    pre_file = os.path.join(SAVE_DIR, "labeled_tweets_with_sentiments.xlsx")
    post_file = os.path.join(SAVE_DIR, "post_election_sentiment_analysis.xlsx")
...
async def constituency_level_analysis():
    try:
        ...
        # Define enhanced party keywords and phrases for exact matching
        parties_keywords = {
            'IND/PTI': ['pti', 'imran khan', 'ik', '804', 'imran', 'khan', 'ind', 'qaidi no', 'tehreek e insaf', 'pakistan tehreek e insaf'],
            'PMLN': ['pmln', 'nawaz sharif', 'maryam nawaz', 'shehbaz sharif', 'muslim league'],
            'PPP': ['ppp', 'bhutto', 'bilawal bhutto', 'zardari', 'peoples party', 'people party', 'jiyala'],
        ...
        # Optional: Save processed data or return as needed
        processed_file_path = os.path.join(SAVE_DIR, 'processed_constituency_analysis.xlsx')
        combined_tweets_df.to_excel(processed_file_path, index=False)

        # Return the path of the saved file or any relevant information
        return JSONResponse(content={"message": "Analysis complete", "file_path": processed_file_path, "plots": plot_images})
...
# Set the full paths to your model files
vectorizer_path = r"D:\Election sentiment analysis (Fast api)\models\tfidf_vectorizer.pkl"
model_path = r"D:\Election sentiment analysis (Fast api)\models\rf_multi_model.pkl"

try:
    vectorizer = joblib.load(vectorizer_path)
    rf_multi_model = joblib.load(model_path)
except Exception as e:
...
@app.post("/predict")
async def predict_party_and_sentiment(input: TextInput):
    try:
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "user", "content": f"Analyze the following text and predict the party affiliation and sentiment: {input.text}"}
            ]
        )
        predictions = response['choices'][0]['message']['content']
        party_prediction, sentiment_prediction = predictions.split(',')

        return {
            "party": party_prediction.strip(),
            "sentiment": sentiment_prediction.strip()
        }
    except Exception as e:
        # Log the error to understand the issue
        print("Error occurred:", e)  # This will print the error in the console
